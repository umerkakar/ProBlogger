							...
							if ( has_action( 'woocommerce_account_downloads_column_' . $column_id ) ) {
								...
								switch ( $column_id ) {
									...
									case 'download-remaining' :
										echo is_numeric( $download['downloads_remaining'] ) ? esc_html( $download['downloads_remaining'] ) : __( '&infin;', 'protheme' );
										break;
...
<?php else : ?>
	<div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
		<a class="btn btn-outline-primary" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Go Shop', 'protheme' ) ?>
		</a>
		<?php esc_html_e( 'No downloads available yet.', 'protheme' ); ?>
	</div>
<?php endif; ?>
